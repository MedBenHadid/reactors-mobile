    ...
    public MembershipRequestForm(String title, Form previous, int assId) {
        ...
        confirm.addActionListener(e->{
            if(tr.getText().length()<8){
                ...
                try {
                    mC.createMembership(new Membership(assId,UserService.getInstance().getUser().getId(),3,"LIVREUR",tr.getText(),Membership.REQUEST_PENDING,Display.getInstance().getLocationManager().getCurrentLocation().getLongitude(),Display.getInstance().getLocationManager().getCurrentLocation().getLatitude()));
