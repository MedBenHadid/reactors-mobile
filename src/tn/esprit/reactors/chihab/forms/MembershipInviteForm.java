    ...
    public MembershipInviteForm(String title, Form previous, int userId) {
        ...
        confirm.addActionListener(e->{
            if(tr.getText().length()<8){
                ...
                try {
                    mC.createMembership(new Membership(AssociationService.getInstance().findAssByManager(UserService.getInstance().getUser().getId()).getId(),userId,3,"LIVREUR",tr.getText(),Membership.INVITE_PENDING,Display.getInstance().getLocationManager().getCurrentLocation().getLongitude(),Display.getInstance().getLocationManager().getCurrentLocation().getLatitude()));
